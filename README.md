...
Another algorithm idea would be to start from the destination and work back by identifying parallel walls to the movement corridor. If the parallel wall has distance 0 from the corridor, it's now a new destination for the final robot. For each distance square > 0, the squares between the wall and the corrider would be a destination for another robot.